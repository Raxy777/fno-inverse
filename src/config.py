...
# ---------------------------------------------------------------------------
N_PML_NET: int = 15
N_PML_FINE: int = N_PML_NET * DOWNSAMPLE            # = 30
PML_ORDER: float = 2.0
PML_R_TARGET: float = 1.0e-4
N_FINE_TOTAL: int = N_FINE + 2 * N_PML_FINE         # = 316
...
R_MAX_LS: float = 1.2
EPS_INTERFACE_CELLS: float = 1.5       # sigmoid width, in network cells
...
EPS_INVERT_START: float = 2.0
EPS_INVERT_END: float = 1.0

# Void model.  See README "Documented deviations".  Stiffness is nulled, density
# is NOT, because c = sqrt(stiffness/rho) blows through the CFL limit if density
# vanishes faster than stiffness.  losses.py reads this same constant so that
# delta_rho in the Lippmann-Schwinger residual matches the solver exactly.
VOID_DENSITY_SCALE: float = 1.0        # 1.0 => density unchanged inside the void
...
# ~6 MB of material and field arrays on the 316^2 padded grid plus ~5 MB of phasor
...
GATE_PML_RESIDUAL: float = 1.0e-4        # step 3: residual/peak energy
...
GATE_GRAD_SIGFIGS: int = 3               # step 9: autodiff vs finite differences
...
SCREEN_CHUNK: int = 64                 # ~50 MB per chunk
N_SURVIVORS: int = 16
...
# PML profile (§3.6)
# ---------------------------------------------------------------------------
def pml_d0(thickness_cells: int = N_PML_FINE, dx: float = DX_FINE,
           c: float = CP, p: float = PML_ORDER, r_target: float = PML_R_TARGET) -> float:
    """d_0 = -(p+1) c ln(R_target) / (2 L_pml)."""
    l_pml = thickness_cells * dx
    return -(p + 1.0) * c * math.log(r_target) / (2.0 * l_pml)
...
def self_check(verbose: bool = True) -> None:
    ...
    basin_envelope = N_CYCLES * lam_s_ref / 2.0
    screen_spacing = L_DOMAIN / SCREEN_GRID
    assert screen_spacing < basin_envelope, (
        f"screen spacing {screen_spacing:.2f} exceeds envelope basin "
        f"{basin_envelope:.2f}: coverage is not guaranteed")
    ok(f"inversion        : waveform basin lam_s/4 = {basin_waveform:.3f} "
       f"({to_mm(basin_waveform):.1f} mm), envelope basin "
       f"{basin_envelope:.2f} ({to_mm(basin_envelope):.0f} mm), "
       f"screen spacing {screen_spacing:.2f} ({to_mm(screen_spacing):.0f} mm) "
       f"over {SCREEN_GRID}x{SCREEN_GRID}={SCREEN_GRID**2} candidates")

    # screen memory footprint, §8.4(d)
    mem_mb = SCREEN_CHUNK * C_IN * N_NET * N_NET * 4 / 1e6
    ok(f"screen memory    : {SCREEN_CHUNK} x {C_IN} x {N_NET}^2 x 4 B = "
       f"{mem_mb:.0f} MB per chunk, "
       f"{SCREEN_GRID**2 // SCREEN_CHUNK} chunks")
    ...
    # -- absorber ----------------------------------------------------------
    ok(f"absorber         : {N_PML_FINE} fine cells = "
       f"{N_PML_FINE*DX_FINE:.2f} lambda_p, p={PML_ORDER:.0f}, "
       f"R_target={PML_R_TARGET:g}, d0={pml_d0():.2f}")
