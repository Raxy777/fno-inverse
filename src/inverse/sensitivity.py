...
def verify_with_solver(result: InversionResult, case: InverseCase, *,
    ...
    This function reports three separately declared criteria rather than calling a
    position-only truth check an independent solver confirmation:

    * `position_gate_pass` uses `GATE_SOLVER_VERIFY_LS`;
    * `residual_gate_pass` uses the solver residual ratio and
      `GATE_SOLVER_VERIFY_RESIDUAL_RATIO`;
    * `shape_gate_pass` uses IoU and, for ellipse fits, the axis-ratio and orientation
      gates.  Two-void matching is already permutation-invariant inside `InversionResult`.

    `independent_status` is ``"pass"``, ``"fail"`` or ``"incomplete"``.  It is pass
    only when every required criterion is measured and passes; missing truth, a
    truth-level numerical residual, or an undefined required metric is incomplete,
    never a pass.  `gate_pass` is retained as a compatibility alias for the aggregate
    result and is `None` for incomplete cases.
    ...
    estimate, correct or not.  There `misfit_solver` and `surrogate_optimism` remain
    useful measurements, but the independent residual criterion is explicitly
    ...
    residual_ratio = None if th_true is None or truth_at_floor else j[0] / j[1]
    position_gate_pass = None if pos is None else bool(pos <= gate)
    residual_gate_pass = (None if residual_ratio is None else
                          bool(residual_ratio <= cfg.GATE_SOLVER_VERIFY_RESIDUAL_RATIO))

    # Shape is a required independent criterion whenever truth is available.  IoU is
    # defined across families; ellipse-specific metrics are required only when both
    # sides are ellipses, because orientation is undefined for a circle.
    if th_true is None:
        shape_gate_pass = None
    else:
        iou = result.iou()
        shape_gate_pass = None if iou is None else bool(iou >= cfg.GATE_IOU)
        if shape_gate_pass is not None and result.axis_ratio_error is not None:
            shape_gate_pass = shape_gate_pass and (
                result.axis_ratio_error <= cfg.GATE_SOLVER_VERIFY_AXIS_RATIO)
        if shape_gate_pass is not None and result.orientation_error_deg is not None:
            shape_gate_pass = shape_gate_pass and (
                result.orientation_error_deg <= cfg.GATE_SOLVER_VERIFY_ORIENTATION_DEG)

    criteria = (position_gate_pass, residual_gate_pass, shape_gate_pass)
    if any(v is False for v in criteria):
        independent_status = "fail"
    elif any(v is None for v in criteria):
        independent_status = "incomplete"
    else:
        independent_status = "pass"
    aggregate = None if independent_status == "incomplete" else independent_status == "pass"
    ...
    return dict(
        ...
        residual_gate=float(cfg.GATE_SOLVER_VERIFY_RESIDUAL_RATIO),
        residual_gate_pass=residual_gate_pass,
        # Shape metrics are scored with InversionResult's family-aware, permutation-
        # invariant implementations rather than raw parameter columns.
        iou=result.iou(), axis_ratio_error=result.axis_ratio_error,
        orientation_error_deg=result.orientation_error_deg,
        truth_family=fam_true.name,
        shape_gate_iou=float(cfg.GATE_IOU),
        shape_gate_axis_ratio=float(cfg.GATE_SOLVER_VERIFY_AXIS_RATIO),
        shape_gate_orientation_deg=float(cfg.GATE_SOLVER_VERIFY_ORIENTATION_DEG),
        ...
        position_error_ls=pos, gate=float(gate), position_gate_pass=position_gate_pass,
        independent_status=independent_status, gate_pass=aggregate,
